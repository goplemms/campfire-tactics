...
storage cap **5** · morale 2 · difficulty normal. *(D74: Vale carries two trap-kits and knows
**Set Trap** from L1. **D79: the cap is now 5 — exactly the 5 starting slots** (salve/stimulant/
antidote pack to 1 each; the 2 trap-kits cost 2), so the stash starts **full** and storage is
felt from step one. The L2 traveler-gift then overflows it, forcing the discard lesson. Tuned
tight on purpose — the run-wide squeeze is the storage through-line.)*
...
storage limit**, act on the overworld, and cash your growth. No combat. Three beats under one
roof:
- **The storage lesson — the traveler's gift (`provision-choice`, D79).** No longer a pick-one.
  A roadside traveler **presses gifts on the party unconditionally** — **2 trap kits + the iron
  weapons** — a grant that **always lands, even over the storage cap** (D75). The bundle starts
  **exactly full** (cap 5), so the gift forces a **discard back to the cap** before Break Camp
  (the discard menu / headless `autoTrim`), *whether or not* the player spent traps at Node 1
  (using them only frees slots; the 3-slot gift still overflows — a discard of **1–3** items).
  - The iron weapons are the **`iron-weapons` party-gear** material (D78) — *carrying* them is
    the party-wide +attack edge — so the discard is a real trade: **keep the buff, the snares,
    or shed the pre-Medic medical dead-weight?** The honest rule the game now shares: *grants
    don't vanish; you choose what to let go.*
  - **Cook a Stew** — *only if Pip is alive* — takes the gifts **and** banks **+2 RP** (the L1
    rescue paying forward; the Cook payoff rides *alongside* the gift, not instead of it).
...
- **Teaches:** **storage-as-a-felt-limit** (grants always land → you choose what to drop; a
  party buff competes with utility for slots); the **overworld-action surface** +
  intel-as-investment (scout to see farther); and that **leveling unlocks abilities you cash at
  a clearing**.
...
- **L2 is now the *storage* lesson — the traveler's gift (D79).** The pick-one became an
  **unconditional traveler-gift that overflows**: the bundle cap dropped **10→5** so the 5
  ...
  weapons** (the D78 party-gear material) — a grant that **always lands over the cap** (D75) —
  forcing a **discard back to the cap** at Break Camp (a discard of **1–3**, robust across
  trap-usage: spending traps at Node 1 only frees slots, and the 3-slot gift still overflows).
  Iron-weapons being a *carried* party-gear item makes the discard a real trade (**keep the
  +attack buff, the snares, or shed pre-Medic medical?**). The Cook-Stew payoff is kept — it now
  rides *alongside* the gift (takes the gifts **and** banks RP). All on the systems already in
  `main`: grant→overflow→discard (D75), party-gear iron-weapons (D78). Guards: tsc · 898 unit
  tests (incl. gift-lands-over-cap + emptied-stash-still-overflows) · build · e2e · sim (Hollow
  Mill completes, storagePressure lever engaged). See **D79**.
