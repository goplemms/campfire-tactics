...
|---|-----------|-----------------|----------------------|
| R1 | **Hardening — the log tells the whole truth + permanent guards** | #111, #115, #116, #122, #124, #136 | A scripted battle using **rescue and Medic heal replays byte-identically** from its log; a JSON round-trip of the log reconstructs it (skill-by-id); the snapshot tripwire test **fails by name** when a mutable field is added unlisted; `streamFor` accepts only registry labels; the two retired deployment models are gone; the active-unit highlight visibly follows the turn. |
