...
const STEPS = BOOT === "battle"
  ...
  : [
      ...
      { name: "06d-influence-patronize", eval: wrap(`s.enterCamp(s.loop.reachable()[0]);for(const jid of ['noble','banker']){const m=(s.guild?s.guild.roster:[]).find(u=>u.jobId===jid);if(m&&!s.run.party.includes(m))s.run.party.push(m);}s.run.camp.purse=120;s.run.overworld.influence=16;s.renderCamp();`) },
      { name: "07-break-camp-gate", eval: wrap(`s.run.camp.purse=0;s.run.overworld.debt=30;s.setOutToMap();`) },
    ];
