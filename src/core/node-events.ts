...
import { recruitClassify, type RecruitOutcome, type Unit } from "./units";
...
// The outcome shape lives in its own leaf so the authored-event modules (`stories.ts`,
// `hollow-mill-events.ts`) can build one without importing this registry, which imports them
// (design map, step 2). Types re-exported: this stays the public home for readers.
import { emptyOutcome, type EventKind, type EventOutcome } from "./event-outcome";
export type { EventKind, EventOutcome } from "./event-outcome";
