...
// `isConcealedTrap` / `traverseRoute` / `THE_HOLLOW_MILL` / `AI` / `manhattan` left with the
// Node-3 beats when they moved to `content/hollow-mill-nodes.test.ts` (D122) — this file's
// remaining cases run on procedural runs, which need no authored body.
...
// The **Node-3 beats** (D81 hold / D82 sweep / D83 tier-3 read / D84 skittish exit) moved to
// `content/hollow-mill-nodes.test.ts` (D122): each reaches `snares` via `traverseRoute` over the
// real arc, and that walk crosses `e1`, whose body is now `content/levels/e1-skirmish.json` —
// `runEncounter` throws on an id it cannot resolve, and `core/` may never import `content/`.
