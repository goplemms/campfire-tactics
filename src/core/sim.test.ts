...
/**
 * **The authored-arc half of the sim lives in `content/`** (D122). The Hollow Mill's node
 * bodies are migrating to `content/levels/*.json`, and a JSON body only resolves once the
 * content layer has injected it — which `core/` may never import. So the arc run, its trap
 * baseline and its digest lines moved verbatim to `content/hollow-mill-sim.test.ts`, and
 * `npm run sim` runs **both** files. This file keeps the procedural robustness net, which
